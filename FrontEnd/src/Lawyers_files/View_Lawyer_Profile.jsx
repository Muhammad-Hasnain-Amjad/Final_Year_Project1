// src/Lawyers_files/View_Lawyer_Profile.jsx
// ✅ LAWYER'S OWN PROFILE — they see their own info + edit it
// The "My Chats" button takes the lawyer to /lawyer-chats to see all client conversations

import React, { useState, useEffect } from "react";
import axios from "axios";
import { useParams, useNavigate } from "react-router-dom";
import {
  FaStar, FaMapMarkerAlt, FaCommentDots, FaHome,
  FaCalendarAlt, FaEdit, FaSave, FaUser, FaIdCard,
  FaPhone, FaEnvelope, FaGavel, FaBuilding,
} from "react-icons/fa";
...
const customSelectStyles = {
  control: (base, state) => ({
    ...
    padding: "4px", borderRadius: "0.75rem", color: "#fff",
    "&:hover": { borderColor: "#facc15" },
  }),
  menu:      (base) => ({ ...base, backgroundColor: "#1f1f1f", borderRadius: "0.75rem", overflow: "hidden", zIndex: 9999, border: "1px solid #3f3f3f" }),
  menuList:  (base) => ({ ...base, backgroundColor: "#1f1f1f", padding: "4px" }),
  option:    (base, state) => ({ ...base, backgroundColor: state.isFocused ? "#facc15" : "#1f1f1f", color: state.isFocused ? "#000" : "#fff", cursor: "pointer", borderRadius: "0.5rem", margin: "2px 4px", "&:active": { backgroundColor: "#eab308" } }),
  singleValue: (base) => ({ ...base, color: "#fff" }),
  input:       (base) => ({ ...base, color: "#fff" }),
  placeholder: (base) => ({ ...base, color: "#9ca3af" }),
  multiValue:       (base) => ({ ...base, backgroundColor: "#facc15", borderRadius: "0.5rem" }),
  multiValueLabel:  (base) => ({ ...base, color: "#000", fontWeight: "600", padding: "4px 8px" }),
  multiValueRemove: (base) => ({ ...base, color: "#000", borderRadius: "0 0.5rem 0.5rem 0", ":hover": { backgroundColor: "#eab308", color: "#000" } }),
};

// ── Small icon-button with tooltip ────────────────────────────────────────────
const IconButton = ({ icon, label, onClick, highlight }) => {
    ...
    <div className="relative flex flex-col items-center">
      <motion.button
        ...
        className={`p-3 rounded-full shadow-lg hover:shadow-xl transition-all ${
          highlight
            ? "bg-gradient-to-r from-yellow-400 to-yellow-500 text-black ring-2 ring-yellow-300"
      ...
      {tip && (
        <motion.div
          ...
          className="absolute -bottom-8 bg-gray-900 text-yellow-400 text-xs px-2 py-1 rounded-md whitespace-nowrap shadow-md z-10"
        >
          {label}
        </motion.div>
      )}
...
function View_Lawyer_Profile() {
  ...
  const [formData, setFormData]       = useState({
    about: "", fee: "", membershipType: "",
    officeAddress: "", practiceAreas: [], courtLevel: "",
  });
  ...
  const handleProfile = async () => {
    ...
    try {
      setEditLoading(true);
      const result = await axios.patch(`${BaseURL}/idlawyer/${id}`, {
          ...
          <div className="flex flex-col lg:flex-row justify-between gap-8 pb-8">
            ...
            >
              <div className="flex gap-3 flex-wrap justify-center items-center">
                ...
                <IconButton icon={<FiLogOut />} label="Logout" onClick={() => {localStorage.removeItem("lawyertoken");
                   localStorage.removeItem("lawyerName"); 
                   localStorage.removeItem("lawyerId");
                    localStorage.removeItem("userType");
                     navigate("/");}} />
                {/* ✅ MY CHATS — takes the lawyer to their chat inbox */}
                <IconButton
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => change("courtLevel", e.target.value)}
                    disabled={!editMode}
                    className="w-full p-3 rounded-xl bg-gray-900 border border-gray-700 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 outline-none text-white disabled:opacity-60 transition-all"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => change("officeAddress", e.target.value)}
                    disabled={!editMode}
              ...
              <div>
                ...
                <Select
                  ...
                  menuPosition="fixed"
                  value={practiceAreaOptions.filter((o) =>
                    formData.practiceAreas.includes(o.value)
                  )}
                  onChange={(sel) =>
                    change("practiceAreas", sel ? sel.map((s) => s.value) : [])
                  }
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 text-black ${
                  editMode
                    ? "bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-lg"
                    : "bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 shadow-lg"
                }`}
              >
